...
function add(userId){
    ...
    ajax.open('POST', 'AddServlet', true);
    let json = JSON.stringify({userid:userId,name: names[0],desc:descriptions[0],value:values[0]});
    ajax.send(json);*/
    $.post("AddServlet",
        {userid:userId,name: names[0],desc:descriptions[0],value:values[0]},
        {
        }
        );
